...
namespace TacticWar.TW_Landscape
{
    ...
    class CZonaBD
    {
        ...
        public CField[,] mField;        //массив - поле боя
        public int mFieldWidth;         //размер поля

        public int mWidth;              //ширина поля боя
        public int mHeight;             //высота поля боя

        public EZemShema mZemShema;     //схема земли

        public SBellmanParam mPutParams; //параметры кратчайшего пути
        ...
        /// <returns></returns>
        public CZonaBD(string fileName)
        {
            //mZemShema = EZemShema.Leto;
            /* !!! */ mFieldWidth = 21;
            ...
            mPutParams.kratPut = new List<CField>();
        }
        ...
        private bool loadMap(string mapFileName)
        {
            try
            {
                ...
                using (StreamReader sr = new StreamReader(mapFileName))
                {
                    ...
                    mField = new CField[mHeight, mWidth];

                    //загрузить схему земли
                    switch (int.Parse(sr.ReadLine()))
                    {
                        case 1:
                            mZemShema = EZemShema.Zima;
                            break;
                        case 2:
                            mZemShema = EZemShema.Gorod;
                            break;
                        case 0:
                        default:
                            mZemShema = EZemShema.Leto;
                    ...
                    for (int k = 0; k < mHeight; k++)
                    {
                        ...
                        {
                            mField[k, l] = new CField(k, l);

                            //определяем тип земли
                            switch(int.Parse(line.Substring(l, 1)))
                            {
                                case 1:
                                    mField[k, l].mZemType = EZemType.zt1_SNEG;
                                    break;
                                case 2:
                                    mField[k, l].mZemType = EZemType.zt2_PESOK;
                                    break;
                                case 3:
                                    mField[k, l].mZemType = EZemType.zt3_VODA;
                                    break;
                                case 4:
                                    mField[k, l].mZemType = EZemType.zt4_KAMNI;
                                    break;
                                case 5:
                                    mField[k, l].mZemType = EZemType.zt5_LES;
                                    break;
                                case 6:
                                    mField[k, l].mZemType = EZemType.zt6_DOROGA;
                                    break;
                                case 7:
                                    mField[k, l].mZemType = EZemType.zt7_STROENIYA;
                                    break;
                                case 8:
                                    mField[k, l].mZemType = EZemType.zt8_LYOD;
                                    break;
                                case 0:
                                default:
                                    mField[k, l].mZemType = EZemType.zt0_ZEMLYA;
                                    break;
                            }
                        }
                    }
        ...
        /// <returns></returns>
        private void BellmanInitPoiskPuti(TW_Units.CElement elem, SCoordinates fC)
        {
        ...
        private bool BellmanCanStep(int i, int j)
        {
            ...
            if ((mField[i, j].mZemType == EZemType.zt3_VODA) &&
        ...
        {
            SCoordinates curCoords; //текущие координаты
        ...
        public bool BellmanPoiskPuti(TW_Units.CElement elem, SCoordinates fC)
        {
            //инициализация
            ...
            //если в ячейке вода, а юнит земной
            if ((mField[fC.x, fC.y].mZemType == EZemType.zt3_VODA) &&
                (!elem.mStepAqua))
                return false;
        ...
        /// <returns></returns>
        public void drawPut(Graphics grf, List<CField> allPut, List<CField> oneDayPut)
        {
        ...
        /// <returns></returns>
        public void fieldZanyatost(TW_Units.CIgrok[] igroki)
        {
            int i, j;
