...
namespace TacticWar.TW_Game
{
    //Симулятор игры - то, что контактирует с граф. интерфейсом и делает все просчёты
    class CSimulator
    {
        //карта
        public TW_Landscape.CZonaBD mGameMap;

        //миссия
        public TW_Mission.CMission mMission;

        //игроки
        public TW_Units.CIgrok[] mMasIgroki;    //массив из двух элементов (0 - игрок 1, 1 - игрок 2)
        ...
        public SUnitInfo mUnitInfo;
        // - информация об атаке
        public SAttackInfo mAttackInfo;
        ...
        SCoordinates mKrestCoords; //координаты установленного креста
        ...
        /// <returns></returns>
        public CSimulator(string misPath, string igrk0Name, string igrk1Name,
                                    bool igrk0AI, bool igrk1AI)
        {
            ...
            mMission = new TW_Mission.CMission(misPath + "mission.mis");
            ...
            mGameMap = new TW_Landscape.CZonaBD(misPath + mapPath);
            ...
            mMasIgroki = new TW_Units.CIgrok[mMission.mCountIgroki];
            ...
            mMasIgroki[0] = new TW_Units.CIgrok(0, igrk0Name, igrk0AI, misPath + "igrok0.igr");
            ...
            mMasIgroki[1] = new TW_Units.CIgrok(1, igrk1Name, igrk1AI, misPath + "igrok1.igr");
            ...
            //mMasIgroki[0].mElements[0].mType = EElementTypes.Infantry;
        ...
        private bool drawPutManager(int igrok, int elemInd, bool countPut)
        {
            ...
            if (mGameMap.mPutParams.kratPut.Count > 0)
            {
                ...
                List<TW_Landscape.CField> oneDayPut = mGameMap.mPutParams.kratPut.ToList<TW_Landscape.CField>();
        ...
        /// <returns></returns>
        public void setUnitInfo(TW_Units.CElement elem)
        {
        ...
        /// <returns></returns>
        public void setUnitInfo(TW_Units.CBuilding building)
        {
            ...
            mUnitInfo.level = ELevels.None; //нет такого поля
        ...
        {
            SCoordinates tmp; //для запоминания координат
        ...
        private void clickEM_pushElemToObject(int igrokId, bool enemieClick, int toFEB, int i, int j)
        {
            ...
            if (mGameMap.mPutParams.kratPut.Count > 0)
            {
                    ...
                    (tmp.y != mMasIgroki[igrokId].mElements[oldSelectInd].mCoords.y))
                {
                    ...
                    List<TW_Landscape.CField> tmpPut = mGameMap.mPutParams.kratPut.ToList<TW_Landscape.CField>();
                    ...
                    mGameMap.mPutParams.kratPut = tmpPut.ToList<TW_Landscape.CField>();
        ...
        /// <param name="j">координата по ширине</param>
        /// <returns>Возвращает сигналы обмена с GUI</returns>
        private ESignals clickElementManager(int igrok, int enemie, bool enemieClick, int ind, int i, int j)
        {
            ...
            switch (switchInt)
            {
                case 1: //передать информацию о выделенном юните
                    ...
                    return ESignals.s03_READY_UNIT_INFO;
        ...
        /// <param name="j">координата по ширине</param>
        /// <returns>Возвращает сигналы обмена с GUI</returns>
        private ESignals clickBuildingManager(int igrok, int enemie, bool enemieClick, int ind, int i, int j)
        {
            int switchInt;
            ...
            switch (switchInt)
            {
                ...
                case 3: //продвинуть выделенный юнит к данному
                    ...
                    if (!enemieClick)
                    {
                        ...
                        {
                            ESignals tmpSig = achievementTag(mMasIgroki[igrok].mElements[oldSelectInd], mMasIgroki[igrok].mBuildings[ind]);
                    ...
                    else
                    {
                        ...
                        {
                            ESignals tmpSig = achievementTag(mMasIgroki[igrok].mElements[oldSelectInd], mMasIgroki[enemie].mBuildings[ind]);
        ...
        /// <param name="j">координата по ширине</param>
        /// <returns>Возвращает сигналы обмена с GUI</returns>
        private ESignals clickEmptyFieldManager(int igrok, int ind, int i, int j)
        {
        ...
        /// <returns>Возвращает сигналы обмена с GUI</returns>
        public ESignals achievementTag(TW_Units.CElement thisElem, TW_Units.CElement tagElem)
        {
        ...
        /// <returns>Возвращает сигналы обмена с GUI</returns>
        public ESignals achievementTag(TW_Units.CElement thisElem, TW_Units.CBuilding tagBuild)
        {
            int tmp;
        ...
        /// <returns>Возвращает сигналы обмена с GUI</returns>
        public ESignals zonaClick(int x, int y)
        {
            int i = y / mGameMap.mFieldWidth;
            int j = x / mGameMap.mFieldWidth;

            //если координаты за границами карты
            if((i < 0) || (i > (mGameMap.mHeight-1))) return ESignals.s05_OUT_OF_RANGE;
            if((j < 0) || (j > (mGameMap.mWidth-1))) return ESignals.s05_OUT_OF_RANGE;
        ...
        //Симулятор атаки
        public int attackSimulator(TW_Units.CElement elem1, TW_Units.CElement elem2,
                List<TW_Units.CElement> podderzh1, List<TW_Units.CElement> podderzh2)
        {
            Random rand = new Random();
            int elem;
            int indUnit1, indUnit2;

            SStructUnits sui1, sui2;
            ...
            while ((elem1.mUnits.Count > 0) && (elem2.mUnits.Count > 0))
            {
                ...
                if (elem2.mUnits[indUnit2].unit.mHealth == EHealth.eh2_DEAD)
                ...
                //удалить юнита, если его убили
                if (elem1.mUnits[indUnit1].unit.mHealth == EHealth.eh2_DEAD)
        ...
        private double getLevelVes(ELevels lev)
        {
        ...
        private void unitAtakUnit(ref SStructUnits structUnit1, ref SStructUnits structUnit2)
        {
            Random rand = new Random();
            ...
            {
                structUnit2.count = 0;
                structUnit2.unit.mHealth = EHealth.eh2_DEAD;
            }
            else if ((wound / (double)structUnit2.unit.mArmourFromInf) < 1)
            {
                structUnit2.count = 1;
                structUnit2.unit.mHealth = EHealth.eh1_WOUNDED;
            }
            else
            {
        ...
        private void recedeElem(TW_Units.CElement elem)
        {
            bool fl = true;
            SCoordinates coords;
                ...
                (mGameMap.mField[coords.x, coords.y - 1].mProhodima))
            {
                ...
                if ((mGameMap.mField[coords.x, coords.y - 1].mZemType == EZemType.zt3_VODA) &&
                    (!elem.mStepAqua))
                    fl = false;
            ...
            {
                //если вода, а юнит не плавает
                if ((mGameMap.mField[coords.x - 1, coords.y].mZemType == EZemType.zt3_VODA) &&
                    (!elem.mStepAqua))
                    fl = false;
